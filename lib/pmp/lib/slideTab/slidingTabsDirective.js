var slidingTabsDirective = angular.module("ionic").directive('ionSlideTabs', ['$timeout', '$compile', '$interval', '$ionicSlideBoxDelegate', '$ionicScrollDelegate',
    '$ionicGesture', '$state', function ($timeout, $compile, $interval, $ionicSlideBoxDelegate, $ionicScrollDelegate, $ionicGesture, $state) {
        return {
            require: "^ionSlideBox",
            restrict: 'A',
            link: function (scope, element, attrs, parent) {
                ...
                var init = function () {
                    ...
                    if (slideHandle) {
                        ...
                        // 判断是否禁用滑动
                        if (attrs.slideTabsScrollable === "false") {
                ...
                var slideToCurrentPosition = function (slideChange) {
                    if (!angular.isDefined(slideTabs) || slideTabs.length == 0) {
                        return false;
                    }
                    ...
                    if (slideChange && attributes.length > 0) {
                        if (tempIndex != targetSlideIndex) {
                            // tab发生变化 触发对应slideChange或者slideGo事件
                            tempIndex = targetSlideIndex;
                            afterTouch(tempIndex);
                        }
                    }
                };
                ...
                var afterTouch = function (index) {
                    var flag = false;
                    for (var i = 0; i < attributes.length; i++) {
                        if (attributes[i].slideGo != null) {
                            flag = true;
                            break;
                        }
                    }
                    if (attributes[index].ionSlideTabLabel == "最新动态") {
                        // 最新动态
                    } else if (flag) {
                        // 跳转页面
                        $state.go(attributes[index].slideGo);
                    } else {
                        var arr = attributes[index].slideChanged.split(",");
                        scope.slideHasChanged(arr[0], arr[1], arr[2]);
                    }
                };
                ...
                scope.addAttributes = function ($attr) {
                    attributes.push($attr);
                };

                scope.onSlideChange = function (slideIndex) {
                    slideToCurrentPosition(true);
                };
            ...
            controller: ['$scope', function ($scope) {
                ...
                this.addAttributes = function ($attrs) {
                    if ($scope.addAttributes) {
                        $scope.addAttributes($attrs);
                    }
                };
            }]
        };
...
slidingTabsDirective.directive('slideChanged', [function () {
    return {
        require: "^ionSlideTabs",
        link: function ($scope, $element, $attrs, $parent) {
...
slidingTabsDirective.directive('slideGo', [function () {
    return {
        require: "^ionSlideTabs",
        link: function ($scope, $element, $attrs, $parent) {
            $parent.addAttributes($attrs);
