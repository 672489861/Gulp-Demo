angular.module('app.quality.check')
    .controller('qualityCheckAddController', ['$scope', '$state',
        '$stateParams','$ionicPopup', 'qualityCheckAddService',
        '$filter','$ionicHistory','$ionicActionSheet',
        ...
        function ($scope, $state,
            ...
            $scope.checkData.checkDate = now.getFullYear()+'-'+qualityCheckAddService.prefixInteger(now.getMonth()+1,2)+'-'+qualityCheckAddService.prefixInteger(now.getDate(),2);
            ...
            $scope.$on('$ionicView.beforeEnter', function () {
                ...
                $scope.checkDataShow = qualityCheckAddService.getCheckDataShow();
                $scope.contentList = qualityCheckAddService.getContent();
                $scope.checkDataShow.checker = qualityCheckAddService.getUserName();
                qualityCheckAddService.setChecker($scope.checkDataShow.checker);
            });
            ...
            $scope.checkSheet = function(){
                ...
                if(checkData.checkPosName == undefined || checkData.checkPosName == ''){
                    flag = false;
                    $ionicPopup.alert({
                        title: '验证',
                        template: '<p class="text-center">未填写检查位置!</p>'
                    });
                }
