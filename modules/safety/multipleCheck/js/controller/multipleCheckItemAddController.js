angular.module('app.safety.multipleCheck')
    .controller('multipleCheckItemAddController',[
        ...
        '$filter','YTService','LocalStorageService',
        function ($scope,$state, $ionicPopup,multipleCheckItemAddService,
            ...
            $scope.$on('$ionicView.beforeEnter', function () {
                ...
                if($stateParams.editId != null && $stateParams.projectId != null){
                    $scope.editId = $stateParams.editId;
                    $scope.projectId = $stateParams.projectId;
                    multipleCheckItemAddService.setEditId($scope.editId);
                    multipleCheckItemAddService.setProjectId($stateParams.projectId);
                    multipleCheckItemAddService.queryDetailData($scope.editId,$stateParams.projectId,function(){
                        $scope.detail = multipleCheckItemAddService.getDetailData();
                        $scope.detail.projectStatus ='';
                        $scope.detail.spotCount =-1;
                        $scope.detail.advice ='';
                        $scope.initDate();
                    });
                    $scope.groupList = [];
                    multipleCheckItemAddService.setGroupList($scope.groupList);
                }else{
                ...
                multipleCheckItemAddService.setType('add');
                    ...
                    $ionicActionSheet.show({
                        ...
                        buttonClicked: function(index) {
                            ...
                            if(index == 2){
                                ...
                                var localDataList = JSON.parse(LocalStorageService.get('multipleCheckList'));
                                var localData={
                                ...
                                if(localDataList == undefined){
                                    ...
                                    for(var k=0;k<localDataList.length;k++){
                                        if(localDataList[k].id > maxId){
                                            maxId = localDataList[k].id;
                                        }
                                    }
                                    localData.id = maxId+1;
                                    localDataList.push(localData);
                                }
                                LocalStorageService.set('multipleCheckList',JSON.stringify(localDataList));
                                $ionicPopup.alert({
                                    title: '验证',
            ...
            $scope.back = function () {
                $ionicPopup.confirm({
                    title: '提示',
                    template: '<p class="text-center">您确定要退出新增页面？</p>',
                    cancelText: '取消',
                    okText: '确定'
                }).then(function(res) {
                    if(res) {
