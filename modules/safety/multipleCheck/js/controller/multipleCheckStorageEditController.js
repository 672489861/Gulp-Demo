        ...
        function ($scope,$state, $ionicPopup,multipleCheckItemAddService,
            ...
            $scope.$on('$ionicView.beforeEnter', function () {
                ...
                if($stateParams.id != null){
                    multipleCheckItemAddService.getStorageData($stateParams.id,function(data){
                        multipleCheckItemAddService.setEditId($stateParams.id);
                        multipleCheckItemAddService.setDetailData(data.detailData);
                        multipleCheckItemAddService.getEditExtraCheckerName(data.detailData.extraCheckerName);
                        multipleCheckItemAddService.setSpot(data.spot);
                        multipleCheckItemAddService.setTempDetail(data.tempDetail);
                        multipleCheckItemAddService.setPosition(data.position);
                        multipleCheckItemAddService.setGroupList(data.groupDataList);
                    });
                }
                $scope.detail = multipleCheckItemAddService.getDetailData() || {};
                $scope.groupList = multipleCheckItemAddService.getGroupDataList() || [];
                $scope.initDate();
                multipleCheckItemAddService.setType('storage');
                ...
                if($scope.validate()){
                    $ionicActionSheet.show({
                        ...
                        buttonClicked: function(index) {
                            ...
                            if(index == 2){
                                var detailData = multipleCheckItemAddService.getDetailData() || {},
                                    spot = multipleCheckItemAddService.getSpot() || [],
                                    position = multipleCheckItemAddService.getPosition() || [],
                                    tempDetail = multipleCheckItemAddService.getTempDetail() || {},
                                    groupDataList = multipleCheckItemAddService.getGroupDataList() || [];
                                var localData={
                                    id:multipleCheckItemAddService.getEditId(),
                                ...
                                multipleCheckItemAddService.setStorageData(localData,function(){
                                    $ionicPopup.alert({
                                        title: '提示',
                ...
                if(data.position.length > 0){
                    multipleCheckItemAddService.insertAddPosition(data.position,function(){
                        $scope.getPosition(data,function(){
                            if(data.historyFlag){
                                multipleCheckItemAddService.insertHistory(data.history,function(){
            ...
            $scope.postData = function(data,msg){
                multipleCheckItemAddService.insertCheckData(data.mainData,data.slave,function(data){
                    if(data.status ==200){
                        $ionicPopup.alert({
                            title: '提示',
                            template: '<p class="text-center">'+msg+'!</p>'
                        }).then(function(){
                            multipleCheckItemAddService.delStorageData(multipleCheckItemAddService.getEditId(),function(){
                                $scope.back();
                            });
                        });
                    }else{
            ...
            $scope.back = function () {
                ...
                $state.go('safety/multipleCheck/multipleCheck-storage-list',{projectId:$scope.detail.projectId,backUrl:'safety/multipleCheck/multipleCheck-item-list'});
