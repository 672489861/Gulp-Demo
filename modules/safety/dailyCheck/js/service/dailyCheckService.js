angular.module('app.safety.dailyCheck')
    .factory("dailyCheckService", ['YTService', 'UserService','env','$ionicActionSheet', function (YT, userService,env,$ionicActionSheet) {
        var serviceData = [],
            pageIndex = 0,
            pageSize = 10,
            hasNextPage = true,
        ...
        return {
            loadListData: function (successCallback) {
                ++pageIndex;
                var self = this;
                right.checker = YT.haveRight(15001002);
                right.solver = YT.haveRight(15001003);
                right.rechecker = YT.haveRight(15001004);
                YT.query({
            ...
            getSearchData: function () {
                var data = {
                    m: 15001001,
                    t: 'v_safety_dailycheck_processing',
                    order: 'id desc',
                    page: pageIndex,
                    rows: pageSize
                };
            ...
            getSearchFilter: function () {
                ...
                if(condition.checkType != -1){
                    if(condition.checkType == 1){
                        filter.push({field: 'checkTypeId', value:1, operator: '=', relation: 'AND'});
                    }else if(condition.checkType == 2){
                        filter.push({field: 'checkTypeId', value:2, operator: '=', relation: 'AND'});
                    }
                }
            ...
            queryDetailRecheckAttach:function(id, successCallback){
                ...
                var data = {
                    m: 15001001,
                    t: 'safety_dailycheck_recheck_attach'
                };
                var filter = [{field: 'id', value: id, operator: '=', relation: 'and'}];
                data.filter = JSON.stringify(filter);
