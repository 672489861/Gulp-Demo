angular.module('app.safety.dailyCheck')
    .controller('dailyCheckAddController', ['$scope', '$state', '$stateParams','$ionicPopup', 'dailyCheckAddService', '$filter','$ionicHistory','$ionicActionSheet','LocalStorageService','$ionicViewSwitcher','YTService',
        function ($scope, $state, $stateParams, $ionicPopup,dailyCheckAddService,$filter,$ionicHistory,$ionicActionSheet,LocalStorageService,$ionicViewSwitcher,YTService) {
            var now = new Date();
            $scope.checkData = dailyCheckAddService.getCheckData();
            $scope.checkData.checkDate = now.getFullYear()+'-'+dailyCheckAddService.prefixInteger(now.getMonth()+1,2)+'-'+dailyCheckAddService.prefixInteger(now.getDate(),2);
            ...
            $scope.$on('$ionicView.beforeEnter', function () {
                ...
                $scope.checkDataShow = dailyCheckAddService.getCheckDataShow();
                $scope.contentList = dailyCheckAddService.getContent();
                $scope.checkDataShow.checker = dailyCheckAddService.getUserName();
                dailyCheckAddService.setChecker($scope.checkDataShow.checker);
